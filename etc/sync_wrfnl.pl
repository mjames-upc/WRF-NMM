...
if (! -e "$wrfnl"){
  die "$wrfnl not found! Is yoru MOAD_DATAROOT correct and have you
...
while ($dom_index < $num_domains) {
  ...
  if ($dom_index == 0){
    ...
    $dx_dom = floor (($dx_dom * 1000.)+.5)/1000.;
    my $dy_dom = ${dy_all}[$parent_ind]/$ratio;
    $dy_dom = floor (($dy_dom * 1000.)+.5)/1000.;
...
# RAR - now that the sync is complete make sure a copy of the wrf.nl file
# gets placed in the domains/<domain> directory for use.
#
system("cp $wrfnl $wrfnl".".bak");
my @tmp    = split /\//, $moad_dataroot;
my $rundir = pop @tmp;
system "cp $wrfnl $ENV{DATA_DOMS}/$rundir/wrf.nl";
# END RAR
